...
onMounted(() => {
  ...
  syncMediaWinState()
  statStore.online = navigator.onLine
})
...
useEventListener('online', () => {
  statStore.online = true
})
useEventListener('offline', () => {
  statStore.online = false
})
...
useIpcRendererOn('mediaWindowShown', () => {
  usePresentStore().mediaScreenInit = true
  ipcRenderer.send('startMediaDisplay', getAllPrefs())
})
useIpcRendererOn('mediaWindowVisibilityChanged', (_e, status: string) => {
  usePresentStore().mediaScreenVisible = status === 'shown'
})
