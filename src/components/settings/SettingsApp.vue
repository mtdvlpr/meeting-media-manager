...
<template>
  <v-form ref="appForm" v-model="valid">
    ...
    <input-field id="app.obs.enable" v-model="app.obs.enable" field="switch">
      <template #label>
        <span v-html="$t('enableObs')" />
      </template>
    </input-field>
    <template v-if="app.obs.enable">
      <input-field id="app.obs.useV4" v-model="app.obs.useV4" field="switch">
        <template #label>
      ...
      <input-field
        ...
        :explanation="$t('obsZoomSceneExplain')"
        :disabled="cameraScenes.length === 0"
        clearable
      />
    </template>
    ...
    <input-field
      ...
      field="switch"
      :explanation="$t('enableZoomExplain')"
    >
      <template #label>
...
const autoStartMeeting = useComputedLabel<AppPrefs>(
  ...
  app,
  'zoom',
  DEFAULT_PREFS.app.zoom.autoStartTime!,
  'autoStartTime',
)
...
const oldName = ref<string | null>(DEFAULT_PREFS.app.congregationName)
watchDebounced(
